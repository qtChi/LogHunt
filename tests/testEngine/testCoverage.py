...
class TestGetCoverageMatrix:
    ...
    def test_has_sigma_rule_true_when_yaml_contains_technique_id(self):
        """Rule YAML mentioning T1078 → T1078 flagged. Check is case-sensitive."""
    ...
    def test_duplicate_class_uid_rules_not_duplicated_in_class_uids(self):
        """
        If two rules for the same technique share the same class_uid,
        class_uids should not contain duplicates.
        """
        from loghunter.engine.mitre_mapper import MappingRule
        rule_a = MappingRule("T1078", 6003, "rule a", lambda e: True)
        rule_b = MappingRule("T1078", 6003, "rule b", lambda e: False)
        rules_by_class = {6003: [rule_a, rule_b], 1001: [], 3001: [], 3002: [], 4001: []}
        engine = _make_engine(rules_by_class=rules_by_class)
        matrix = engine.get_coverage_matrix()
        t1078 = next((r for r in matrix if r.technique_id == "T1078"), None)
        if t1078:
            assert t1078.class_uids.count(6003) == 1
        engine = _make_engine()
        matrix = engine.get_coverage_matrix()
...
class TestGetCoverageSummary:
    ...
    def test_sigma_confirmed_count_correct_when_some_covered(self):
        # yaml "T1059" exactly matches only T1059 — subtechniques T1059.001 and
        # T1059.007 are NOT substrings of "T1059" so they don't match.
        engine = _make_engine(confirmed_yamls=["T1059"])
        summary = engine.get_coverage_summary()
        assert summary["sigma_confirmed_count"] == 1
...
class TestGetTechniquesForTactic:
    ...
    def test_tactic_with_sigma_coverage_reflected(self):
        # yaml "T1059" matches T1059 exactly. T1059.001 and T1059.007 don't
        # match because "T1059.001" is not a substring of "T1059".
        # Use a yaml that contains all three to test full tactic coverage.
        engine = _make_engine(confirmed_yamls=["T1059 T1059.001 T1059.007"])
        result = engine.get_techniques_for_tactic("Execution")
        for record in result:
